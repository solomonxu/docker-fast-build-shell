...
function ssh_remote_trust_concur()
{
    ...
    _wait_seconds=2
	for ((_times=1; _times<${_hosts_count}; _times++)); do
        sleep ${_wait_seconds}
...
## execute COMMAND on remote host
function ssh_remote_exec_stream()
{
    if [ $# -lt 3 ]; then
        logger_warn "Call ssh_remote_exec_stream, but lack of arguments: host, command and fifo_fullname."
        return 1
    fi
    _host=$1
    _command=$2
	_fifo_fullname=$3
    logger_debug "Call ssh_remote_exec_stream ${_host} ${_command} > ${_fifo_fullname}."
    $(ssh ${_host} "${_command}" > ${_fifo_fullname})
}
